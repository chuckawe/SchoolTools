...
term = 'T2'
print 'Reading Course grades for',term

cur_read = CourseCurrentReader('data/Grades_1.24.2014.xlsx')
#cur_read = CourseCurrentReader('data/DPCourseCurrent.xlsx')
cur_read.get_courses(students,final_term=term)  # SET THIS TO TAKE THE DESIRED TERM
...
for stu in students:
    print stu.last,stu.first
    grade = stu.get_hs_grade()
    if grade > 12:
        grade = 12
    for course in stu.get_courses(2013):
        if 'Physical Education' in course.title: continue
        if 'College Readiness' in course.title: continue
        if 'Theatre' in course.title: continue
        print course.title
        course.numAvg = course.term_grades[term]
        #print course.numAvg
